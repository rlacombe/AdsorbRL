...
class EnvironmentLoopHer(core.Worker):
  ...
  def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        state, action, reward, next_state, goal, step_type, it = episode_experience[timestep]
        ...
        if her_type == HERType.FINAL:
            ...
            it=len(episode_experience)
            final_goal = episode_experience[-1][3]
            new_reward = env_reward_function(final_goal)
  ...
  def run_episode(self) -> loggers.LoggingData:
    ...
    while not timestep.last():
      ...
      episode_return = tree.map_structure(operator.iadd,
                                          episode_return,
                                          timestep.reward)
      transition = (old_state,action,episode_return,timestep.observation,self._environment.goal,timestep.step_type,episode_steps-1)
      ...
      self.update_replay_buffer(
        ...
        episode_experience,
        her_type=HERType.FINAL,
        env_reward_function= self._environment.rewardFunctionPrediction,
        num_relabeled=self._num_relabeled
        )
    # Record counts.
    counts = self._counter.increment(episodes=1, steps=episode_steps)
    for i in range(episode_steps):
      state, action, reward, next_state, step_type_new,it = self._replay_buffer.sample()
  ...
  def run(
    ...
    with signals.runtime_terminator():
      while not should_terminate(episode_count, step_count):
        ...
        self._replay_buffer = experience_buffer.Buffer(1e6, 128)
